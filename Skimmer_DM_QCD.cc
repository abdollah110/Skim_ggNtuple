   ...
   fChain->SetBranchStatus("m*",1); 
    ...
    for (int jentry=0; jentry<nentries;jentry++) {
        ...
        for (int imu = 0; imu < nMu; ++imu){
            ...
            if (muPt->at(imu) > 30 &&  fabs(muEta->at(imu)) < 2.4 &&   muIsLooseID->at(imu) > 0  && IsoMu < 1.0 ){
                Mu4Momentum.SetPtEtaPhiM(muPt->at(imu),muEta->at(imu),muPhi->at(imu),MuMass);
                
                for (int ijet = 0; ijet < nJet; ++ijet){
Jet4Momentum.SetPtEtaPhiM(jetPt->at(ijet),jetEta->at(ijet),jetPhi->at(ijet),jetEn->at(ijet));
	if (jetPt->at(ijet) > 30 &&  fabs(jetEta->at(ijet)) < 3.0   && Jet4Momentum.DeltaR(Mu4Momentum) > 0.5 ){
                            isMuJet=1;
                            break;
