...
void Skimmer::Loop(TString outputName, int skm)
{
	...
	bool isEleMu=0;
        ...
        for (int iele = 0; iele < nEle; ++iele){
            ...
            if (elePt->at(iele) > 20 && fabs(eleEta->at(iele))< 2.5 &&  IsoEle < 0.15)
            {isEle=1;
                hcount->Fill(3);
 Ele4Momentum.SetPtEtaPhiM(elePt->at(iele),eleEta->at(iele),elePhi->at(iele),eleMass);
        ...
        for (int imu = 0; imu < nMu; ++imu){
            ...
            if (muPt->at(imu) > 20 &&  fabs(muEta->at(imu)) < 2.4 &&   muIsLooseID->at(imu) > 0  && IsoMu < 0.15 )
            {isMu=1;
                hcount->Fill(4);
Mu4Momentum.SetPtEtaPhiM(muPt->at(imu),muEta->at(imu),muPhi->at(imu),MuMass);
        ...
        for (int itau = 0; itau < nTau; ++itau){
            if (tauPt->at(itau) > 20 && fabs(tauEta->at(itau)) < 2.3 && tauCombinedIsolationDeltaBetaCorrRaw3Hits->at(itau) < 0.5 * tauPt->at(itau)  && tauByLooseMuonRejection3->at(itau) &&  tauByMVA6LooseElectronRejection->at(itau) ){
...
realName=token;
    ...
    Skimmer t("root://cmsxrootd.fnal.gov//store/user/abdollah/Crab3/ggNtuple/V1/"+FinaName);
